...
function updateLoansUI() {
    ...
    list.innerHTML = (Data.loansData || []).map(loan => `
        <div class="flex justify-between items-center p-3 bg-white rounded-xl border border-slate-200 hover:border-[#B8964B]/50 transition-all group shadow-sm">
            <div class="flex items-center gap-3">
                ${loan.proof ? `<div class="w-8 h-8 rounded bg-slate-100 overflow-hidden border border-slate-200" onclick="window.app.viewProof('${loan.proof}')"><img src="${loan.proof}" class="w-full h-full object-cover"></div>` : `<div class="w-8 h-8 rounded bg-slate-100 flex items-center justify-center border border-slate-200"><i data-lucide="file-text" class="w-3.5 h-3.5 text-slate-400"></i></div>`}
                <div>
                    <h4 class="text-[9px] font-bold text-slate-900 uppercase tracking-wider">${loan.entity}</h4>
                    <p class="text-[7px] text-slate-500 uppercase tracking-widest mt-0.5">${loan.type} • ${loan.dueDate || 'No Due Date'}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <div class="text-right">
                    <p class="text-[10px] font-black ${loan.type === 'Receivable' ? 'text-emerald-600' : 'text-rose-600'}">${parseFloat(loan.amount).toLocaleString()}</p>
                    <span class="text-[6px] font-black uppercase px-1.5 py-0.5 rounded ${loan.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'} border border-current opacity-80">${loan.status}</span>
                </div>
                <button onclick="window.app.deleteLoan('${loan.id}')" class="w-6 h-6 flex items-center justify-center rounded-lg bg-rose-50 text-rose-500 hover:bg-rose-100 transition-all opacity-0 group-hover:opacity-100">
...
function updateSalesUI() {
    ...
    tableBody.innerHTML = allRecords.map(r => `
        <tr class="border-t border-slate-100 hover:bg-slate-50 transition-colors group">
            <td class="p-3">
                <div class="flex items-center gap-2">
                    <span class="text-[7px] font-black px-1.5 py-0.5 rounded ${r.type === 'SALE' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'} border border-current opacity-80">${r.type}</span>
                    <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">#${r.id}</span>
                </div>
            </td>
            <td class="p-3">
                <div class="flex items-center gap-2">
                    ${r.proof ? `<div class="w-6 h-6 rounded bg-slate-100 overflow-hidden border border-slate-200" onclick="window.app.viewProof('${r.proof}')"><img src="${r.proof}" class="w-full h-full object-cover"></div>` : ''}
                    <div>
                        <h4 class="text-[10px] font-bold text-slate-900 uppercase">${r.product}</h4>
                        <p class="text-[7px] text-slate-500 uppercase">${r.shop}</p>
                    </div>
                </div>
            </td>
            <td class="p-3 text-right">
                <p class="text-[10px] font-black text-slate-900">${r.amount.toLocaleString()}</p>
                <span class="text-[7px] font-black ${r.type === 'SALE' ? 'text-emerald-600' : 'text-slate-400'} uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">${r.type === 'SALE' ? '+' + r.profit.toLocaleString() : r.date}</span>
            </td>
        </tr>
    `).join('');
}
...
export function getSalesPage() {
    return `
        <div class="h-full flex flex-col gap-4 overflow-hidden">
            ...
            <div class="shrink-0 luxury-card p-4 space-y-4 border-slate-200 bg-white shadow-sm">
                <div class="flex justify-between items-center border-b border-slate-100 pb-2">
                    <h3 class="text-[10px] font-black text-[#B8964B] uppercase tracking-[0.3em] flex items-center gap-2">
                        <i data-lucide="shield-check" class="w-3 h-3"></i> Secure Transaction Terminal
                    </h3>
                    <select id="finance-mode" class="bg-slate-50 border border-slate-200 rounded px-2 py-0.5 text-[8px] font-black text-[#B8964B] uppercase outline-none">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1">
                        ...
                        <select id="sale-shop" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-[10px] text-slate-900 focus:border-[#B8964B]/50 outline-none">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" id="sale-product" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-[10px] text-slate-900 focus:border-[#B8964B]/50 outline-none" placeholder="Enter details">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1">
                        <label class="text-[7px] font-black text-slate-500 uppercase ml-1">Market Value</label>
                        <input type="number" id="sale-price" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-[10px] text-slate-900 focus:border-[#B8964B]/50 outline-none" placeholder="0.00">
                    </div>
                    <div class="flex items-end gap-2">
                        <input type="file" id="finance-proof-input" class="hidden" accept="image/*">
                        <button onclick="document.getElementById('finance-proof-input').click()" class="flex-1 h-[34px] bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-center text-slate-500 hover:text-slate-900 transition-all">
                            <i data-lucide="camera" class="w-4 h-4"></i>
                        </button>
                        <button id="btn-add-sale" class="flex-[2] h-[34px] bg-gradient-to-r from-[#B8964B] to-[#D4AF37] text-white font-black text-[9px] uppercase tracking-widest rounded-lg shadow-lg active:scale-95 transition-all">
                            Authorize
                        </button>
                    </div>
                </div>

                <div id="finance-proof-wrapper" class="hidden flex items-center gap-3 p-2 bg-slate-50 rounded-lg border border-slate-200">
                    <img id="finance-proof-preview" class="w-10 h-10 rounded object-cover border border-[#B8964B]/30">
                    <p class="text-[7px] font-bold text-emerald-600 uppercase tracking-widest">Image Attached (Securely Cached)</p>
                </div>
            </div>

            <div class="flex-1 overflow-y-auto custom-scrollbar pr-1">
                <div class="luxury-card border-slate-200 bg-white">
        ...
        <div class="h-full flex flex-col gap-4 overflow-hidden">
            ...
            <div class="shrink-0 luxury-card p-4 space-y-4 border-slate-200 bg-white shadow-sm">
                ...
                <div class="flex gap-2">
                    ...
                    <button onclick="document.getElementById('loan-proof-input').click()" class="w-12 h-[34px] bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-center text-slate-500 hover:text-slate-900 transition-all">
                        <i data-lucide="camera" class="w-4 h-4"></i>
                    </button>
                    <button id="btn-add-loan" class="flex-1 h-[34px] bg-gradient-to-r from-[#B8964B] to-[#D4AF37] text-white font-black text-[9px] uppercase tracking-widest rounded-lg shadow-lg active:scale-95 transition-all">
                        Register Portfolio
                    </button>
                </div>
            ...
            <div class="shrink-0 grid grid-cols-3 gap-3">
                <div class="luxury-card p-4 bg-emerald-50 border-emerald-100">
                    <p class="text-[7px] font-black text-emerald-600 uppercase tracking-widest">Receivable</p>
                    <h3 id="total-receivable" class="text-lg font-bold text-slate-900 mt-1">0</h3>
                </div>
                <div class="luxury-card p-4 bg-rose-50 border-rose-100">
                    <p class="text-[7px] font-black text-rose-600 uppercase tracking-widest">Liabilities</p>
                    <h3 id="total-payable" class="text-lg font-bold text-slate-900 mt-1">0</h3>
                </div>
                <div class="luxury-card p-4 bg-blue-50 border-blue-100">
                    <p class="text-[7px] font-black text-blue-600 uppercase tracking-widest">Net Position</p>
                    <h3 id="total-net" class="text-lg font-bold text-slate-900 mt-1">0</h3>
                </div>
            </div>

            <div class="flex-1 overflow-y-auto custom-scrollbar pr-1">
                <div class="luxury-card p-4 bg-white border-slate-200 shadow-sm">
